...
async function getAcceptedSubmissions(handle, count = 100) {
  const submissions = await getUserStatus(handle, 1, count);

  return submissions.filter(sub => sub.verdict === 'OK');
}

async function getSolvedProblemTags(handle, count = 1000) {
  const accepted = await getAcceptedSubmissions(handle, count);
  const tags = new Set();
  accepted.forEach(sub => {
    if (sub.problem.tags) sub.problem.tags.forEach(i => tags.add(i));
  });
...
async function getSolvedProblemDifficultyStats(handle, count = 1000) {
  ...
  accepted.forEach(sub => {
    ...
    if (!diffMap[rating]) diffMap[rating] = 0;
    diffMap[rating]++;
  });
...
async function getAverageSolvedProblemRating(handle, count = 1000) {
  const accepted = await getAcceptedSubmissions(handle, count);
  const ratings = accepted.map(sub => sub.problem.rating).filter(rating => rating);
  if (!ratings.length) return null;
...
async function getUnsolvedAttemptedProblems(handle, count = 1000) {
  ...
  const unsolved = [...attempted].filter(key => !solved.has(key));
...
async function getSolveAttemptsMap(handle, count = 1000) {
  ...
  return Object.entries(tries)
    ...
    .map(([key, val]) => ({ problemId: key, attempts: val.attempts }));
}
...
async function findProblemSolveById(handle, contestId, index) {
  const submissions = await getUserStatus(handle, 1, 1000);
